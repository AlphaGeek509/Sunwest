...
Public Class frmWO
    ...
    Public Sub subLoadAdministration(ByVal intWorkOrderID As Integer, ByVal strPassingForm As String)
        ...
        If strPassingForm = "TEMPLATE" Then
            strSQL = "SELECT CORE_NO, PO_NO, CUBIC_INCH, FREIGHT_CORE_PUR, FREIGHT_OUTGOING, FREIGHT_RETURN, FREIGHT_OWN, BLOCK_NO, CRANK_NO, " & _
                          "HEAD_NO, CON_ROD_CORE, CAM_CORE, CRANK_CORE, BLOCK_CORE_CHARGE, HEAD_CORE, FREIGHT_CARRIER, TYPE, AUTO_MODEL, AUTO_VIN_CODE, DESCRIPTION, NOTES, FREIGHT_COST, JOBBER_COST " & _
                          "FROM TEMPLATE " & _
                          "WHERE ID = '" & intWorkOrderID & "' "
        ElseIf strPassingForm = "WO" Then
            strSQL = "SELECT CUSTOMER_ID, CORE_NO, PO_NO, CUBIC_INCH, WO_DATE, BLOCK_NO, CRANK_NO, HEAD_NO, CON_ROD_CORE, CAM_CORE, CRANK_CORE, " & _
                                      ...
                                      "FREIGHT_OUTGOING, FREIGHT_RETURN, STATUS, FREIGHT_OWN, TYPE, SHELF_NUMBER, AUTO_MODEL, AUTO_VIN_CODE, TEMPLATE_DESC, FREIGHT_COST, JOBBER_COST " & _
                                "FROM WORK_ORDER " & _
        ...
        If dsLoadAdmin.Tables(0).Rows.Count = 1 Then
            ...
            Else
                rtxtNotes.Text = Nothing
            End If
            If Not IsDBNull(dsLoadAdmin.Tables(0).Rows(0).Item("FREIGHT_COST")) Then
                txtFreightCost.Text = dsLoadAdmin.Tables(0).Rows(0).Item("FREIGHT_COST")
            Else
                txtFreightCost.Text = Nothing
            End If
            If Not IsDBNull(dsLoadAdmin.Tables(0).Rows(0).Item("JOBBER_COST")) Then
                txtJobber.Text = dsLoadAdmin.Tables(0).Rows(0).Item("JOBBER_COST")
            Else
                txtJobber.Text = Nothing
            End If
    ...
    Public Sub subInsertWO(ByVal intWorkOrderID As Integer)
        ...
        If Not IsNumeric(txtFreightCost.Text) Then
            MsgBox("Please enter in a numerical value only for freight cost.", MsgBoxStyle.OkOnly, "Currency Only")
            Exit Sub
        End If
        If Not IsNumeric(txtJobber.Text) Then
            MsgBox("Please enter in a numerical value only for jobber cost.", MsgBoxStyle.OkOnly, "Currency Only")
            Exit Sub
        End If
        ...
        strSQL = "INSERT WORK_ORDER(CORE_NO, PO_NO, CUBIC_INCH, WO_DATE, AUTO_MAKE, AUTO_YEAR, FREIGHT_CORE_PUR, FREIGHT_OUTGOING, FREIGHT_RETURN, BLOCK_NO, CRANK_NO, HEAD_NO, CON_ROD_CORE, " & _
                            ...
                            "AUTO_MODEL, AUTO_VIN_CODE, TEMPLATE_DESC, FREIGHT_COST, JOBBER_COST) " & _
                      "VALUES('" & cboCore.Text & "', '" & txtPO.Text & "', '" & txtCubic.Text & "', '" & Format(dtWODate.Value, "yyyy-MM-dd HH:MM:ss") & "', '" & cboItemMake.ValueMember & "', '" & cboItemYear.ValueMember & "'," & _
                            ...
                            "'" & txtModel.Text & "', '" & txtVinCode.Text & "', '" & strTemplateDesc & "', '" & txtFreightCost.Text & "', '" & txtJobber.Text & "')"
    ...
    Public Sub subPrintWO(ByVal strLocalorEmail As String, ByVal intWorkOrder As Integer)
        Dim strSQL As String
        ...
        Dim rptWorkOrder As New rptWorkOrder
        Dim CrExportOptions As ExportOptions
        Dim CrDiskFileDestinationOptions As New DiskFileDestinationOptions()
        Dim CrFormatTypeOptions As New PdfRtfWordFormatOptions()


        strSQL = "SELECT CUSTOMER_ID, CORE_NO, PO_NO, CUBIC_INCH, WO_DATE, MAKE_DESC, AUTO_YEAR, " & _
                            "BLOCK_NO, CRANK_NO, HEAD_NO, CON_ROD_CORE, CAM_CORE, CRANK_CORE, BLOCK_CORE_CHARGE, SHELF_NUMBER, " & _
                            "NEEDED_BY, INVOICE_NO, NOTES, FREIGHT_CORE_PUR, FREIGHT_OUTGOING, FREIGHT_RETURN, HEAD_CORE, FREIGHT_DESC, AUTO_MODEL, AUTO_VIN_CODE " & _
                      "FROM WORK_ORDER, AUTO_MAKE, FREIGHT_CARRIERS " & _
                      "WHERE WORK_ORDER_ID = '" & intWorkOrder & "' AND " & _
                            "AUTO_MAKE = MAKE_VALUE AND " & _
                            "FREIGHT_VALUE = FREIGHT_CARRIER"

        dsPrint = fctGetDataSet(strSQL)

        rptWorkOrder.DataDefinition.FormulaFields("WORK_ORDER_ID").Text = "'" & intWorkOrder & "'"

        '- Administration Items -'
        If dsPrint.Tables(0).Rows.Count > 0 Then
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("CUSTOMER_ID")) Then
                rptWorkOrder.DataDefinition.FormulaFields("CUSTOMER_ID").Text = "'" & dsPrint.Tables(0).Rows(0).Item("CUSTOMER_ID") & "'"
            End If
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("CORE_NO")) Then
                rptWorkOrder.DataDefinition.FormulaFields("CORE_NO").Text = "'" & dsPrint.Tables(0).Rows(0).Item("CORE_NO") & "'"
            End If
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("PO_NO")) Then
                rptWorkOrder.DataDefinition.FormulaFields("PO_NO").Text = "'" & dsPrint.Tables(0).Rows(0).Item("PO_NO") & "'"
            End If
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("CUBIC_INCH")) Then
                rptWorkOrder.DataDefinition.FormulaFields("CUBIC_INCH").Text = "'" & dsPrint.Tables(0).Rows(0).Item("CUBIC_INCH") & "'"
            End If
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("WO_DATE")) Then
                rptWorkOrder.DataDefinition.FormulaFields("WO_DATE").Text = "'" & Format(dsPrint.Tables(0).Rows(0).Item("WO_DATE"), "MM/dd/yyyy") & "'"
            End If
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("MAKE_DESC")) Then
                rptWorkOrder.DataDefinition.FormulaFields("AUTO_MAKE").Text = "'" & dsPrint.Tables(0).Rows(0).Item("MAKE_DESC") & "'"
            End If
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("AUTO_YEAR")) Then
                rptWorkOrder.DataDefinition.FormulaFields("AUTO_YEAR").Text = "'" & dsPrint.Tables(0).Rows(0).Item("AUTO_YEAR") & "'"
            End If
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("BLOCK_NO")) Then
                rptWorkOrder.DataDefinition.FormulaFields("BLOCK_NO").Text = "'" & dsPrint.Tables(0).Rows(0).Item("BLOCK_NO") & "'"
            End If
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("CRANK_NO")) Then
                rptWorkOrder.DataDefinition.FormulaFields("CRANK_NO").Text = "'" & dsPrint.Tables(0).Rows(0).Item("CRANK_NO") & "'"
            End If
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("HEAD_NO")) Then
                rptWorkOrder.DataDefinition.FormulaFields("HEAD_NO").Text = "'" & dsPrint.Tables(0).Rows(0).Item("HEAD_NO") & "'"
            End If
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("CON_ROD_CORE")) Then
                rptWorkOrder.DataDefinition.FormulaFields("CON_ROD_CORE").Text = "'" & dsPrint.Tables(0).Rows(0).Item("CON_ROD_CORE") & "'"
            End If
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("CAM_CORE")) Then
                rptWorkOrder.DataDefinition.FormulaFields("CAM_CORE").Text = "'" & dsPrint.Tables(0).Rows(0).Item("CAM_CORE") & "'"
            End If
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("CRANK_CORE")) Then
                rptWorkOrder.DataDefinition.FormulaFields("CRANK_CORE").Text = "'" & dsPrint.Tables(0).Rows(0).Item("CRANK_CORE") & "'"
            End If
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("BLOCK_CORE_CHARGE")) Then
                rptWorkOrder.DataDefinition.FormulaFields("BLOCK_CORE_CHARGE").Text = "'" & dsPrint.Tables(0).Rows(0).Item("BLOCK_CORE_CHARGE") & "'"
            End If
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("NEEDED_BY")) Then
                rptWorkOrder.DataDefinition.FormulaFields("NEEDED_BY").Text = "'" & dsPrint.Tables(0).Rows(0).Item("NEEDED_BY") & "'"
            End If
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("INVOICE_NO")) Then
                rptWorkOrder.DataDefinition.FormulaFields("INVOICE_NO").Text = "'" & dsPrint.Tables(0).Rows(0).Item("INVOICE_NO") & "'"
            End If
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("NOTES")) Then
                Dim strNotes As String = dsPrint.Tables(0).Rows(0).Item("NOTES")
                strNotes = strNotes.Replace("-NL-", "' & Chr(10) & '")
                rptWorkOrder.DataDefinition.FormulaFields("NOTES").Text = "'" & strNotes & "'"
            End If
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("FREIGHT_DESC")) Then
                If dsPrint.Tables(0).Rows(0).Item("FREIGHT_DESC") = "Select A Freight" Then
                    rptWorkOrder.DataDefinition.FormulaFields("FREIGHT_CARRIER").Text = ""
                Else
                    rptWorkOrder.DataDefinition.FormulaFields("FREIGHT_CARRIER").Text = "'" & dsPrint.Tables(0).Rows(0).Item("FREIGHT_DESC") & "'"
                End If

            End If
            'If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("FREIGHT_OUTGOING")) Then
            '    rptWorkOrder.DataDefinition.FormulaFields("FREIGHT_OUTGOING").Text = "'" & dsPrint.Tables(0).Rows(0).Item("FREIGHT_OUTGOING") & "'"
            'End If
            'If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("FREIGHT_RETURN")) Then
            '    rptWorkOrder.DataDefinition.FormulaFields("FREIGHT_RETURN").Text = "'" & dsPrint.Tables(0).Rows(0).Item("FREIGHT_RETURN") & "'"
            'End If
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("HEAD_CORE")) Then
                rptWorkOrder.DataDefinition.FormulaFields("HEAD_CORE_CHARGE").Text = "'" & dsPrint.Tables(0).Rows(0).Item("HEAD_CORE") & "'"
            End If
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("AUTO_MODEL")) Then
                rptWorkOrder.DataDefinition.FormulaFields("AUTO_MODEL").Text = "'" & dsPrint.Tables(0).Rows(0).Item("AUTO_MODEL") & "'"
            End If
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("AUTO_VIN_CODE")) Then
                rptWorkOrder.DataDefinition.FormulaFields("AUTO_VIN_CODE").Text = "'" & dsPrint.Tables(0).Rows(0).Item("AUTO_VIN_CODE") & "'"
            End If
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("SHELF_NUMBER")) Then
                rptWorkOrder.DataDefinition.FormulaFields("SHELF_NUMBER").Text = "'" & dsPrint.Tables(0).Rows(0).Item("SHELF_NUMBER") & "'"
            End If
        End If
        ...
        dsPrint = Nothing
        strSQL = "SELECT WORK_ORDER_ID, TEMPLATE_DESC " & _
                      "FROM WORK_ORDER " & _
                            "LEFT OUTER JOIN TEMPLATE " & _
                                "ON TEMPLATE_DESC = DESCRIPTION " & _
                      "WHERE WORK_ORDER_ID = '" & intWorkOrder & "'"

        dsPrint = fctGetDataSet(strSQL)

        If dsPrint.Tables(0).Rows.Count > 0 Then
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("TEMPLATE_DESC")) Then
                rptWorkOrder.DataDefinition.FormulaFields("TEMPLATE_DESC").Text = "'" & dsPrint.Tables(0).Rows(0).Item("TEMPLATE_DESC") & "'"
            End If
        End If
        ...
        '- Ship To Address -'
        dsPrint = Nothing
        strSQL = "SELECT CUST_ID, FIRST_NAME, LAST_NAME, ADDR_1, ADDR_2, ADDR_3, CITY, STATE, ZIPCODE, DAY_PHONE, COMPANY " & _
                      "FROM CUSTOMER_SHIP_TO, WORK_ORDER " & _
                      "WHERE WORK_ORDER_ID = '" & intWorkOrder & "' AND " & _
                            "CUSTOMER_ID = CUST_ID"
        ...
        If dsPrint.Tables(0).Rows.Count > 0 Then
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("COMPANY")) Then
                rptWorkOrder.DataDefinition.FormulaFields("SHIP_COMPANY").Text = "'" & StrConv(dsPrint.Tables(0).Rows(0).Item("COMPANY"), VbStrConv.ProperCase) & "'"
            End If
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("FIRST_NAME")) Then
                rptWorkOrder.DataDefinition.FormulaFields("SHIP_CUSTOMER_NAME").Text = "'" & StrConv(dsPrint.Tables(0).Rows(0).Item("FIRST_NAME"), VbStrConv.ProperCase) & " " & StrConv(dsPrint.Tables(0).Rows(0).Item("LAST_NAME"), VbStrConv.ProperCase) & "'"
            End If

            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("ADDR_1")) Then
                If dsPrint.Tables(0).Rows(0).Item("ADDR_1") <> Nothing Then
                    strAddress = dsPrint.Tables(0).Rows(0).Item("ADDR_1")
                End If
            End If

            '- Insert Address 2 -'
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("ADDR_2")) Then
                If dsPrint.Tables(0).Rows(0).Item("ADDR_2") <> Nothing Then
                    If Len(strAddress) > 0 Then
                        strAddress += ControlChars.CrLf & dsPrint.Tables(0).Rows(0).Item("ADDR_2")
                    End If
                End If
            End If

            '- Insert Address 3 -'
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("ADDR_3")) Then
                If dsPrint.Tables(0).Rows(0).Item("ADDR_3") <> Nothing Then
                    If Len(strAddress) > 0 Then
                        strAddress += ControlChars.CrLf & dsPrint.Tables(0).Rows(0).Item("ADDR_3")
                    End If
                End If
            End If

            '- Insert Address City -'
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("CITY")) Then
                If dsPrint.Tables(0).Rows(0).Item("CITY") <> Nothing Then
                    If Len(strAddress) > 0 Then
                        strAddress += ControlChars.CrLf & dsPrint.Tables(0).Rows(0).Item("CITY") & ", "
                    End If
                End If
            ...
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("STATE")) Then
                If dsPrint.Tables(0).Rows(0).Item("STATE") <> Nothing Then
                    If Len(strAddress) > 0 Then
                        strAddress += dsPrint.Tables(0).Rows(0).Item("STATE") & " "
                    End If
                End If
            End If

            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("ZIPCODE")) Then
                If dsPrint.Tables(0).Rows(0).Item("ZIPCODE") <> Nothing Then
                    If Len(strAddress) > 0 Then
                        strAddress += CStr(dsPrint.Tables(0).Rows(0).Item("ZIPCODE"))
                    End If
                End If
            End If

            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("DAY_PHONE")) Then
                If dsPrint.Tables(0).Rows(0).Item("DAY_PHONE") <> Nothing Then
                    If Len(strAddress) > 0 Then
                        strAddress += ControlChars.CrLf & dsPrint.Tables(0).Rows(0).Item("DAY_PHONE")
                    End If
                End If
            End If
            rptWorkOrder.DataDefinition.FormulaFields("SHIP_ADDRESS").Text = "'" & Replace(strAddress, ControlChars.CrLf, "' & chr(13) & '") & "'"
        End If
        ...
        dsPrint = Nothing
        strSQL = "SELECT CUST_ID, FIRST_NAME, LAST_NAME, ADDR_1, ADDR_2, ADDR_3, CITY, STATE, ZIPCODE, DAY_PHONE, COMPANY " & _
                      "FROM CUSTOMER_BILL_TO, WORK_ORDER " & _
                      "WHERE WORK_ORDER_ID = '" & intWorkOrder & "' AND " & _
                            "CUSTOMER_ID = CUST_ID"
        ...
        If dsPrint.Tables(0).Rows.Count > 0 Then
            ...
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("COMPANY")) Then
                rptWorkOrder.DataDefinition.FormulaFields("COMPANY").Text = "'" & StrConv(dsPrint.Tables(0).Rows(0).Item("COMPANY"), VbStrConv.ProperCase) & "'"
            End If
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("FIRST_NAME")) Then
                rptWorkOrder.DataDefinition.FormulaFields("CUSTOMER_NAME").Text = "'" & StrConv(dsPrint.Tables(0).Rows(0).Item("FIRST_NAME"), VbStrConv.ProperCase) & " " & StrConv(dsPrint.Tables(0).Rows(0).Item("LAST_NAME"), VbStrConv.ProperCase) & "'"
            End If
            ...
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("DAY_PHONE")) Then
                ...
                End If
            End If
            rptWorkOrder.DataDefinition.FormulaFields("ADDRESS").Text = "'" & Replace(strAddress, ControlChars.CrLf, "' & chr(13) & '") & "'"
        End If
        ...
        dsPrint = Nothing
        strSQL = "SELECT RUN_TIME, ELECTRIC_LOAD, OIL_PRESSURE, COMPRESSION " & _
              "FROM SIM_TEST " & _
              "WHERE WORK_ORDER_ID = '" & intWorkOrder & "'"
        ...
        '- Simulation Tests -'
        If dsPrint.Tables(0).Rows.Count > 0 Then
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("RUN_TIME")) Then
                rptWorkOrder.DataDefinition.FormulaFields("SIM_RUN_TIME").Text = "'" & dsPrint.Tables(0).Rows(0).Item("RUN_TIME") & " mins.'"
            End If
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("ELECTRIC_LOAD")) Then
                rptWorkOrder.DataDefinition.FormulaFields("SIM_LOAD").Text = "'" & dsPrint.Tables(0).Rows(0).Item("ELECTRIC_LOAD") & " amps.'"
            End If
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("OIL_PRESSURE")) Then
                rptWorkOrder.DataDefinition.FormulaFields("SIM_OIL_PRESSURE").Text = "'" & dsPrint.Tables(0).Rows(0).Item("OIL_PRESSURE") & " psi.'"
            End If
            If Not IsDBNull(dsPrint.Tables(0).Rows(0).Item("COMPRESSION")) Then
                rptWorkOrder.DataDefinition.FormulaFields("SIM_COMPRESSION").Text = "'" & dsPrint.Tables(0).Rows(0).Item("COMPRESSION") & " psi.'"
            End If
        Else
            rptWorkOrder.DataDefinition.FormulaFields("SIM_RUN_TIME").Text = "'N/A'"
            rptWorkOrder.DataDefinition.FormulaFields("SIM_LOAD").Text = "'N/A'"
            rptWorkOrder.DataDefinition.FormulaFields("SIM_OIL_PRESSURE").Text = "'N/A'"
            rptWorkOrder.DataDefinition.FormulaFields("SIM_COMPRESSION").Text = "'N/A'"
        End If
        ...
        dtTemp.Columns.Add("SUB_ITEM", System.Type.GetType("System.String"))
        dtTemp.Columns.Add("QUANTITY", System.Type.GetType("System.String"))
        dtTemp.Columns.Add("COST", System.Type.GetType("System.String"))
        Dim dr As DataRow

        Dim srptMachineWork As CrystalDecisions.CrystalReports.Engine.ReportDocument = rptWorkOrder.Subreports(0)

        strSQL = "SELECT WO_ID, MACHINE_WORK_WO.MACHINE_ITEM, QUANTITY, COST " & _
                      "FROM MACHINE_WORK_WO, MACHINE_WORK " & _
                      "WHERE MW_ITEM_ID = MACHINE_ID AND " & _
                            "WO_ID = '" & intWorkOrder & "' " & _
                      "ORDER BY MW_ITEM_ID ASC"


        dsTemp = fctGetDataSet(strSQL)
        Do Until intCtr = dsTemp.Tables(0).Rows.Count
            dr = dtTemp.NewRow

            dr.Item("SUB_ITEM") = dsTemp.Tables(0).Rows(intCtr).Item("MACHINE_ITEM")
            dr.Item("QUANTITY") = dsTemp.Tables(0).Rows(intCtr).Item("QUANTITY")
            dr.Item("COST") = dsTemp.Tables(0).Rows(intCtr).Item("COST")
            intCtr += 1

            dtTemp.Rows.Add(dr)
        Loop

        dsMW.Tables.Add(dtTemp)
        dsMW.Tables(0).TableName = "subMachineWork"
        dsMW.WriteXmlSchema(Directory.GetCurrentDirectory & "\srptMachineWork01.xsd")
        srptMachineWork.SetDataSource(dsMW)



        dsPrint = Nothing
        Dim dsTempMaterial As New DataSet
        Dim intMatCtr As Integer
        Dim dtMatTemp As New DataTable
        dtMatTemp.Columns.Add("SUB_ITEM", System.Type.GetType("System.String"))
        dtMatTemp.Columns.Add("PART_NUMBER", System.Type.GetType("System.String"))
        dtMatTemp.Columns.Add("SIZE", System.Type.GetType("System.String"))
        'Dim dr As DataRow

        Dim srptMaterialUsed As CrystalDecisions.CrystalReports.Engine.ReportDocument = rptWorkOrder.Subreports(1)

        strSQL = "SELECT WO_ID, MATERIAL_USED_WO.MATERIAL_ITEM, PART_NUMBER, SIZE " & _
                      "FROM MATERIAL_USED_WO, MATERIAL_USED " & _
                      "WHERE MU_ITEM_ID = MATERIAL_ID AND " & _
                            "WO_ID = '" & intWorkOrder & "' " & _
                      "ORDER BY MU_ITEM_ID ASC"


        dsTempMaterial = fctGetDataSet(strSQL)
        Do Until intMatCtr = dsTempMaterial.Tables(0).Rows.Count
            dr = dtMatTemp.NewRow

            dr.Item("SUB_ITEM") = dsTempMaterial.Tables(0).Rows(intMatCtr).Item("MATERIAL_ITEM")
            dr.Item("PART_NUMBER") = dsTempMaterial.Tables(0).Rows(intMatCtr).Item("PART_NUMBER")
            dr.Item("SIZE") = dsTempMaterial.Tables(0).Rows(intMatCtr).Item("SIZE")
            intMatCtr += 1

            dtMatTemp.Rows.Add(dr)
        Loop

        dsMU.Tables.Add(dtMatTemp)
        dsMU.Tables(0).TableName = "subMaterialUsed"
        dsMU.WriteXmlSchema(Directory.GetCurrentDirectory & "\srptMaterialUsed01.xsd")
        srptMaterialUsed.SetDataSource(dsMU)



        If strLocalorEmail = "EMAIL" Then
            '- Create a temp directory to export then temp pdf file -'
            If Not Directory.Exists("C:\Sunwest\tmpEmailPDF") Then
                Directory.CreateDirectory("C:\Sunwest\tmpEmailPDF")
            End If

            CrDiskFileDestinationOptions.DiskFileName = "C:\Sunwest\tmpEmailPDF\" & intWorkOrder & ".pdf"

            CrExportOptions = rptWorkOrder.ExportOptions
            ...
            End Try
        ElseIf strLocalorEmail = "PREVIEW" Then
            Dim frmRPTViewer As New frmRPTviewer
            frmRPTViewer.CrystalReportViewer1.ReportSource = rptWorkOrder
            frmRPTViewer.Show()
        Else
            rptWorkOrder.PrintToPrinter(1, False, 1, 1)
        End If
    ...
    Private Sub btnEmailWO_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnEmailWO.Click
        ...
        objclient.HTMLBody = "<html><strong>LYNTRON, INC.</strong><br/></html>"
